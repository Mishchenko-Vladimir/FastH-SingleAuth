...
def create_app(lifespan_override=None) -> FastAPI:
    ...
    app = FastAPI(
        ...
        docs_url=None if settings.site.environment == "production" else "/docs",
        redoc_url=None if settings.site.environment == "production" else "/redoc",
        openapi_url=(
            None if settings.site.environment == "production" else "/openapi.json"
        ),
    )
    ...
    if settings.site.environment != "testing":
        authentication_backend = AdminAuth(
            secret_key=settings.admin.admin_secret_key.get_secret_value()
        )
        admin = Admin(
            app=app,
    ...
    if settings.site.environment != "production":
